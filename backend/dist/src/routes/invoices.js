...
router.get("/", (req, res) => {
    try {
        const { page = "1", limit = "10", search = "", status = "", customerType = "", dateFrom = "", dateTo = "", sortBy = "createdAt", sortOrder = "desc", } = req.query;
        let filteredInvoices = [...mockInvoices];
        if (search) {
            const searchTerm = search.toString().toLowerCase();
            filteredInvoices = filteredInvoices.filter((invoice) => invoice.number.toLowerCase().includes(searchTerm) ||
        ...
        if (customerType) {
            filteredInvoices = filteredInvoices.filter((invoice) => invoice.customerType === customerType);
        }
        if (dateFrom || dateTo) {
            filteredInvoices = filteredInvoices.filter((invoice) => {
...
router.post("/", (req, res) => {
    try {
        const { customerId, customerName, customerEmail, customerType, orderId, issueDate, dueDate, paymentTerms, items, taxRate, discountAmount = 0, notes = "", } = req.body;
        if (!customerId ||
            !customerName ||
            !issueDate ||
            !dueDate ||
            ...
            items.length === 0) {
            res.status(400).json({
                error: "Missing required fields: customerId, customerName, issueDate, dueDate, and items are required",
            });
            return;
        }
        ...
        const newInvoiceNumber = `INV-${String(mockInvoices.length + 1).padStart(3, "0")}`;
        const newInvoice = {
            id: newInvoiceNumber,
    ...
    try {
        ...
        res.json({
            ...
            invoice: {
                ...
                remainingBalance: mockInvoices[invoiceIndex].total -
...
router.delete("/:id", (req, res) => {
    try {
        ...
        if (invoice.paidAmount > 0) {
            res.status(400).json({
                error: "Cannot delete an invoice with payments. Cancel the invoice instead.",
            });
            return;
        }
        mockInvoices[invoiceIndex] = {
