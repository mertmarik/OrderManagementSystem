...
export const settingsController = {
  ...
  getSettingsSection: async (req: Request, res: Response): Promise<void> => {
    try {
  ...
  updateSettingsSection: async (req: Request, res: Response): Promise<void> => {
    try {
      const { section } = req.params;
  ...
  toggleTwoFactor: async (req: Request, res: Response): Promise<void> => {
    try {
  ...
  // Connect integration
  connectIntegration: async (req: Request, res: Response): Promise<void> => {
    try {
      const { integrationId } = req.params;
      const { apiKey, settings: integrationSettings } = req.body;

      // Find integration in available list
      const availableIntegration = mockSettings.integrations.available.find(
      ...
      mockSettings.integrations.available =
        mockSettings.integrations.available.filter(
          (integration) => integration.id !== integrationId
        );
  ...
  disconnectIntegration: async (req: Request, res: Response): Promise<void> => {
    try {
      const { integrationId } = req.params;

      // Find integration in connected list
      const connectedIntegration = mockSettings.integrations.connected.find(
        (integration) => integration.id === integrationId
      );
      ...
      mockSettings.integrations.connected =
        mockSettings.integrations.connected.filter(
          (integration) => integration.id !== integrationId
        );
      mockSettings.integrations.updatedAt = new Date().toISOString();
  ...
  exportSettings: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        'attachment; filename="oms-settings-export.json"'
      );
      res.json(exportData);
    } catch (error) {
  ...
  importSettings: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      // Update settings with imported data (selective import)
